...
<?php foreach($operation_products_grp as $index => $ex){ ?>
    <tr id="<?php echo $index;?>" style="height: 65px;">
		...
		<td><?php echo $Operation->getProductMaster($ex['product']['operation_id'], $index); ?></td>
		...
		<td class="left size_wrapper">
				...
				<?php if($size_group_id > 0){ ?>
					...
					<?php //echo '<pre>'; print_r(var_dump( $size_groups  )); ?>
		...
		<?php $zakup_grn = $ex['product']['operation_zakup'] /
			...
			<?php echo number_format($zakup_grn,2,'.',''); ?> грн</td>
        ...
        <td class="right"><input type="text" class="edit price_invert right" id="price_invert<?php echo $index;?>" style="width:70px;" value="<?php echo number_format($ex['product']['price_invert'],2,'.',''); ?>"> грн</td>
...
<script>
	...
	function setTotalQuantity(row_id){
		...
		$('#summ_'+row_id).html((quantity * zakup)+' '+$('#summ_'+row_id).data('currency_simbol'));
	}
